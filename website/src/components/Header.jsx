...
export default function Header() {
  ...
  // Anchors live on each language's own home page — on privacy.html/terms.html/
  // license.html (and across /tr/ vs root) these need the full path back to
  // that language's home, not a same-page "#features" that does nothing
  // on a page with no such element (or lands on the wrong language).
    ...
    <header className="sticky top-0 z-30 border-b border-stone-800/80 bg-stone-950/85 backdrop-blur-sm">
      <div className="max-w-6xl mx-auto px-6 h-14 flex items-center justify-between">
        <a href={home} className="flex items-center gap-2.5">
          <img src={`${assetBase}logo.png`} alt="" className="h-6 w-6 rounded-md" />
